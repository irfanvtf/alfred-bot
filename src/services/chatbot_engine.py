...
from src.models.session import ConversationMessage, MessageRole, SessionUpdate
from src.models.intent import ChatRequest, ChatResponse, IntentMatch
...
class ChatbotEngine:
    ...
    def process_message(
        ...
        try:
            ...
            response_data = self._select_response(intent_matches, session_context, message)
            
            # Update conversation state
            self._update_conversation_state(session_id, response_data)
            
            # Generate final response with templating
            final_response = self._generate_response_text(response_data, session_context)
            ...
            logger.info(f"Processed message for session {session_id}: {response_data.get('intent_id', 'fallback')}")
    ...
    def _select_response(
        ...
        # Debug logging
        print(f"DEBUG: _select_response called with {len(intent_matches)} matches")
        if intent_matches:
            best_match = intent_matches[0]
            print(f"DEBUG: Best match - Intent: {best_match['intent_id']}, Confidence: {best_match['confidence']}, Threshold: {self.confidence_threshold}")
        ...
        if intent_matches:
            ...
            if best_match["confidence"] >= self.confidence_threshold:
                ...
                print(f"DEBUG: Using high confidence match: {best_match['intent_id']}")
                ...
                print(f"DEBUG: Checking flow appropriateness for medium confidence match")
                if self._is_flow_appropriate(best_match, session_context):
                    print(f"DEBUG: Flow appropriate, using match: {best_match['intent_id']}")
                    return self._prepare_intent_response(best_match, session_context)
                else:
                    print(f"DEBUG: Flow not appropriate for match: {best_match['intent_id']}")
    ...
    def _select_appropriate_response(
        ...
        if intent_match["category"] == "greeting":
            ...
            if message_count <= 1:
                ...
                greeting_responses = [r for r in responses if "help" in r.lower() or "assist" in r.lower()]
    ...
    def _get_fallback_response(
        ...
        conversation_state = session_context.get("conversation_state", ConversationState.ONGOING)
        message_count = session_context.get("message_count", 0)
    ...
    def _generate_response_text(
        ...
        response_text = response_data.get("response", "I'm not sure how to respond to that.")
    ...
    def get_conversation_state(self, session_id: str) -> Optional[ConversationState]:
        ...
        session = session_manager.get_session(session_id)
        if session:
            return session.context_variables.get("conversation_state", ConversationState.GREETING)
        return None
