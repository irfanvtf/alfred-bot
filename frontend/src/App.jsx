...
function App() {
  ...
  const handleSaveIntent = async (intent) => {
    ...
    setError(null)
    try {
      const method = intent.id ? 'PUT' : 'POST'
      const url = intent.id 
  ...
  const handleDeleteIntent = async (intentId) => {
    ...
    setError(null)
    try {
      const response = await fetch(`/api/v1/cms/intents/${language}/${intentId}`, {
        method: 'DELETE'
      })
    ...
    <div className="min-h-screen bg-gray-50">
      <header className="bg-white shadow">
        <div className="max-w-7xl mx-auto px-4 py-6 sm:px-6 lg:px-8 flex justify-between items-center">
          <h1 className="text-3xl font-bold text-gray-900">Alfred Bot CMS</h1>
          <LanguageSelector 
            language={language} 
