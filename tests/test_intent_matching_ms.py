#!/usr/bin/env python3
"""
Comprehensive test script for validating intent matching in Alfred Bot (Malaysian language).
This script tests all intents and patterns defined in the dialog-ms.json file
by calling the /api/v1/chat endpoint with language parameter set to "ms" and 
asserting the correct intent responses.
"""
...
DIALOG_FILE_PATH = "data/sources/ms/dialog-ms.json"
...
def test_intent_matching(intent_id: str, pattern: str, expected_response_id: str) -> bool:
    """
    Test if a given pattern correctly matches to its intent for Malaysian language
    ...
    try:
        ...
        payload = {
            ...
            "user_id": "test_user",
            "language": "ms"  # Specify Malaysian language
        }
        
        # Make API call with language parameter
        response = requests.post(f"{CHAT_ENDPOINT}?language=ms", json=payload)
...
def wait_for_api(max_wait_time: int = 30) -> bool:
    """
    ...
    """
    print(f"⏳ Waiting for API to become available (max {max_wait_time}s)...")
...
def run_comprehensive_intent_tests():
    ...
    print("🧪 Starting comprehensive intent matching tests (Malaysian)...\n")
    
    # Load dialog data
    try:
        dialog_data = load_dialog_data()
        print(f"📚 Loaded dialog data with {len(dialog_data.get('intents', []))} intents")
    except Exception as e:
    ...
    # Print summary
    print("\n" + "="*50)
    print("📊 TEST RESULTS SUMMARY (Malaysian)")
    print("="*50)
    print(f"Total tests:  {test_results['total']}")
    print(f"Passed:       {test_results['passed']}")
    print(f"Failed:       {test_results['failed']}")
    print(f"Success rate: {test_results['passed']/test_results['total']*100:.1f}%" if test_results['total'] > 0 else "No tests run")
...
def main():
    ...
    print("🚀 Alfred Bot - Intent Matching Test Suite (Malaysian)")
    print("="*50)
